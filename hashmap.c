...
void eraseMap(HashMap * map,  char * key) {    
    long posicion;
    for(posicion = hash(key, map->capacity); (posicion < map->capacity); posicion++){
      if((map->buckets[posicion] != NULL) && (is_equal(map->buckets[posicion]->key,key) == 1)){
...
void * firstMap(HashMap * map) {
    long posicion;
    for(posicion = 0; posicion < map->size ;posicion++){
